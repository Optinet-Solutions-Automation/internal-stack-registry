...
import { updatePurchase, type CreatePurchaseInput } from './actions';
...
export default function EditPurchaseModal({ purchase, onClose }: { purchase: Purchase; onClose: () => void }) {
  const [form, setForm] = useState<Form>({
    ...
    warranty_expires: purchase.warranty_expires ?? '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const receipt_url = receiptFile
        ? await uploadReceipt(receiptFile, 'receipts')
        : purchase.receipt_url;
      const warranty_receipt_url = warrantyFile
        ? await uploadReceipt(warrantyFile, 'warranty')
        : purchase.warranty_receipt_url;

      const result = await updatePurchase(purchase.id, { ...form, receipt_url, warranty_receipt_url });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          {/* Receipts */}
          <div className="border-t border-gray-800 pt-4 space-y-3">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Receipts — upload new to replace existing</p>
            ...
            <Field label="Purchase Receipt">
              ...
              >
                {receiptFile ? (
                  ...
                  <p className="text-sm text-green-400">Current receipt on file — click to replace</p>
                ) : (
            ...
            <Field label="Warranty Receipt">
              ...
              >
                ...
                <input ref={warrantyRef} type="file" accept="image/*" className="hidden"
