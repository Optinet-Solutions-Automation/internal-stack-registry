...
function PasswordCell({ password }: { password: string | null }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={() => setVisible(!visible)} className="text-gray-500 hover:text-gray-300 transition-colors">
...
function RotationStatus({ lastRotated, policy }: { lastRotated: string | null; policy: string | null }) {
  ...
  if (!lastRotated) return <span className="text-gray-500">Never rotated</span>;
  ...
  if (policyDays && days !== null && days > policyDays) {
    return <div><span className="text-red-400 font-medium">{label}</span><span className="ml-2 text-xs text-red-400">({days}d ago — overdue)</span></div>;
  }
  if (policyDays && days !== null && days > policyDays * 0.8) {
    return <div><span className="text-yellow-400 font-medium">{label}</span><span className="ml-2 text-xs text-yellow-400">({days}d ago)</span></div>;
  }
...
export default function CredentialsClient({ credentials, tools }: { credentials: CredWithTool[]; tools: ToolOption[] }) {
  const [showModal, setShowModal] = useState(false);
  const [filterType, setFilterType] = useState('');

  const loginTypes = [...new Set(credentials.map(c => c.login_type).filter(Boolean))] as string[];

  const preFiltered = credentials.filter(c => !filterType || c.login_type === filterType);

  const searchFn = useCallback((c: CredWithTool, q: string) =>
    !!(c.tools?.name?.toLowerCase().includes(q) || c.owner?.toLowerCase().includes(q) || c.username?.toLowerCase().includes(q) || c.credential_location?.toLowerCase().includes(q)),
  []);
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      {overdueCount > 0 && (
        <div className="bg-red-900/30 border border-red-800 rounded-xl px-4 py-3 sm:px-5 sm:py-4 flex items-center gap-3">
          ...
          <p className="text-xs sm:text-sm font-semibold text-red-300">{overdueCount} credential{overdueCount > 1 ? 's' : ''} overdue for rotation</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-3 sm:p-4">
          ...
          <p className="mt-1 text-lg sm:text-xl font-bold text-white">{credentials.filter(c => !c.last_rotated).length}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-3">
          ...
          {filterType && (
            <button onClick={() => setFilterType('')} className="text-sm text-gray-400 hover:text-white px-3 py-2 transition-colors">Clear</button>
          )}
        </div>
      </div>
      ...
      <div className="lg:hidden space-y-3">
        {paginated.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-10 text-center text-gray-500">No credentials found</div>
        ) : paginated.map(cred => (
          <div key={cred.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-1">
            <div className="flex items-center justify-between mb-2">
              {cred.tools ? (
                <Link href={`/tools/${cred.tools.id}`} className="font-medium text-indigo-400 hover:text-indigo-300 text-sm">{cred.tools.name}</Link>
              ) : <span className="text-gray-500">—</span>}
              {cred.login_type && (
                <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-gray-800 text-gray-300">{cred.login_type}</span>
              )}
            </div>
            <CardField label="Username"><span className="font-mono text-xs text-gray-300">{cred.username ?? '—'}</span></CardField>
            <CardField label="Password"><PasswordCell password={cred.password} /></CardField>
            <CardField label="Location"><span className="font-mono text-xs text-gray-300">{cred.credential_location ?? '—'}</span></CardField>
            <CardField label="Owner"><span className="text-xs text-gray-400">{cred.owner ?? '—'}</span></CardField>
            <CardField label="Policy"><span className="text-xs text-gray-400">{cred.rotation_policy ?? '—'}</span></CardField>
            <CardField label="Last Rotated"><span className="text-xs"><RotationStatus lastRotated={cred.last_rotated} policy={cred.rotation_policy} /></span></CardField>
          </div>
        ))}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {paginated.length === 0 ? (
                  <tr><td colSpan={8} className="px-5 py-10 text-center text-gray-500">No credentials found</td></tr>
                ) : paginated.map(cred => (
                  <tr key={cred.id} className="hover:bg-gray-800/50 transition-colors">
                    <td className="px-5 py-4">{cred.tools ? <Link href={`/tools/${cred.tools.id}`} className="font-medium text-indigo-400 hover:text-indigo-300">{cred.tools.name}</Link> : '—'}</td>
                    <td className="px-5 py-4">{cred.login_type ? <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-gray-800 text-gray-300">{cred.login_type}</span> : '—'}</td>
                    <td className="px-5 py-4 text-gray-300 font-mono text-xs">{cred.username ?? '—'}</td>
                    <td className="px-5 py-4"><PasswordCell password={cred.password} /></td>
                    ...
                    <td className="px-5 py-4"><RotationStatus lastRotated={cred.last_rotated} policy={cred.rotation_policy} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showModal && <AddCredentialModal tools={tools} onClose={() => setShowModal(false)} />}
    </div>
  );
}
