...
import { updateTool, type UpdateToolInput } from './actions';
import type { BillingType, RiskLevel, ToolStatus, Tool } from '@/types/database';

export default function EditToolModal({ tool, onClose }: { tool: Tool; onClose: () => void }) {
  const [form, setForm] = useState<UpdateToolInput>({
    id: tool.id,
    name: tool.name,
  ...
  const set = (field: keyof UpdateToolInput, value: unknown) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    setError(null);
    const result = await updateTool(form);
    setLoading(false);
    if (result.error) {
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <Field label="Tool Name *">
            <input type="text" value={form.name} onChange={e => set('name', e.target.value)}
              required placeholder="e.g. Stripe" className={inputCls} />
          </Field>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Category">
              <input type="text" value={form.category} onChange={e => set('category', e.target.value)}
                placeholder="e.g. Payments" className={inputCls} />
            </Field>
            <Field label="Vendor">
              <input type="text" value={form.vendor} onChange={e => set('vendor', e.target.value)}
                placeholder="e.g. Stripe Inc." className={inputCls} />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Owner">
              <input type="text" value={form.owner} onChange={e => set('owner', e.target.value)}
                placeholder="e.g. Finance" className={inputCls} />
            </Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Billing Type">
              <select value={form.billing_type} onChange={e => set('billing_type', e.target.value as BillingType)} className={inputCls}>
            ...
            <Field label="Risk Level">
              <select value={form.risk_level} onChange={e => set('risk_level', e.target.value as RiskLevel)} className={inputCls}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Status">
              <select value={form.status} onChange={e => set('status', e.target.value as ToolStatus)} className={inputCls}>
          ...
          <Field label="Description">
            <textarea value={form.description} onChange={e => set('description', e.target.value)}
              rows={3} placeholder="What does this tool do?" className={`${inputCls} resize-none`} />
          </Field>
